 ...
 * to store the data before the telnet connection is established. So its 
 ...
 *     
 *  Wait <colTime> ms before sending a telnet block if its size is less
 *  than <minSize> (defined by setMinBlockSize):
 ...
 *    
 *  Change the message which will be send to the telnet client after
 ...
 * 
 * If you have to store a lot of data before a telnet session can be established you
 * may increase the value of the define TELNETSPY_BUFFER_LEN for a bigger ring buffer.    
 */
...
class TelnetSpy : public Stream {
  public:
    ...
    void setCaptureOsPrint(bool capture);
